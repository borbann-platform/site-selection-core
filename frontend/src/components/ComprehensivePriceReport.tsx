 ...
 * Displays predicted price with feature contributions, market context,
...
function formatFeatureValue(feature: string, value: number): string {
  ...
  if (feature === "district_avg_price_sqm") return `฿${value.toLocaleString()}/sqm`;
...
function generateMockExtendedData(
  ...
  const comparables: ComparableProperty[] = nearbyProperties
    ...
    .map((p) => {
      ...
      if (
        p.building_style_desc === property.building_style_desc
      ) {
        similarity += 15;
      }
      if (
  ...
  const pricePercentile =
    property.total_price && avgComparablePrice
      ? Math.min(
          Math.max(
            50 + ((property.total_price - avgComparablePrice) / avgComparablePrice) * 50,
            5
          ),
          95
        )
