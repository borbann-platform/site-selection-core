...
function requestListingsTile() {
  ...
  const res = http.get(url, { tags: { endpoint: "listings_tile" } });
  check(res, {
...
function requestDetailFlow() {
  ...
  const detailRes = http.get(detailUrl, { tags: { endpoint: "house_price_detail" } });
  check(detailRes, {
    "detail status 2xx/404": (r) => (r.status >= 200 && r.status < 300) || r.status === 404,
  });

  const nearbyRes = http.get(nearbyUrl, { tags: { endpoint: "house_price_nearby" } });
  check(nearbyRes, {
